...
`apiStats_` は90日分の資金予測を2回まわすため、素で20秒前後かかる。
GAS側に結果キャッシュを入れてあり、データが変わっていなければ3秒台で返る。
キャッシュは**時間では切っていない**。取引明細・入金予定の行数、設定シートの中身、
対策シミュレーションから指紋を作り、1文字でも変われば捨てる。
古い残高を新しい残高として見せないための作りなので、時間切れ方式に変えないこと。

GASはときどきJSONではなくHTMLのページを返す（実測で3回に1回程度）。
JSONPだと callback が呼ばれないまま script の読み込みだけ成功するので、
`onload` で取りこぼしを検出して最大3回まで黙って取り直している。
